...
export const ingestIncomeStatements = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (result.error) {
      console.error(`[ingestIncomeStatements] ${result.companyId} ${result.year}Q${result.season}:`, result.error);
    ...
    if (result.warnings.length > 0) {
      console.warn(`[ingestIncomeStatements] ${result.companyId} ${result.year}Q${result.season}:`, result.warnings);
    }
    ...
    res.status(result.skipped ? 200 : 201).json({
      message: result.skipped
        ...
        : 'Successfully ingested income statement.',
...
export const backfillIncomeStatements = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // 終點不是「今年 Q4」，而是依法定公告截止日（Q1 5/15、Q2 8/14、Q3 11/14、Q4 次年 3/31）
    // 找出今天為止最新一筆「應該已公告」的季度，再往前數 years*4 季，確保每一季都是理論上已有資料可抓。
    ...
    console.log(
      `[backfill] Starting backfill for ${companyId} (dataType=${dataType}, force=${force}): ${quarters.length} quarters, ` +
    ...
    for (let i = 0; i < quarters.length; i++) {
      ...
      if (result.skipped) {
        console.log(`[backfill] ${progress} SKIP (already in DB) ${companyId} ${year}Q${season}`);
      } else if (result.success) {
        console.log(`[backfill] ${progress} FETCHED ${companyId} ${year}Q${season}${result.warnings.length ? ` (${result.warnings.length} warnings)` : ''}`);
      } else {
    ...
    console.log(`[backfill] Done. ${succeeded}/${quarters.length} succeeded for ${companyId} (${fetched} fetched from MOPS, ${skipped} already in DB).`);
