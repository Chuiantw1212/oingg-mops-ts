...
export const ingestBalanceSheets = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (result.error) {
      console.error(`[ingestBalanceSheets] ${result.companyId} ${result.year}Q${result.season}:`, result.error);
    ...
    if (result.warnings.length > 0) {
      console.warn(`[ingestBalanceSheets] ${result.companyId} ${result.year}Q${result.season}:`, result.warnings);
    }
    ...
    res.status(result.skipped ? 200 : 201).json({
      message: result.skipped
        ...
        : 'Successfully ingested balance sheet.',
...
export const backfillBalanceSheets = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // 終點季度依法定公告截止日判斷（同損益表規則），往前數 years*4 季，確保每一季理論上都已有資料可抓。
    ...
    console.log(
      `[backfill-balance-sheet] Starting backfill for ${companyId} (dataType=${dataType}, force=${force}): ${quarters.length} quarters, ` +
    ...
    for (let i = 0; i < quarters.length; i++) {
      ...
      if (result.skipped) {
        console.log(`[backfill-balance-sheet] ${progress} SKIP (already in DB) ${companyId} ${year}Q${season}`);
      } else if (result.success) {
        console.log(`[backfill-balance-sheet] ${progress} FETCHED ${companyId} ${year}Q${season}${result.warnings.length ? ` (${result.warnings.length} warnings)` : ''}`);
      } else {
        console.log(`[backfill-balance-sheet] ${progress} NO DATA ${companyId} ${year}Q${season}: ${result.error ?? result.mopsMessage}`);
      }
    ...
    console.log(`[backfill-balance-sheet] Done. ${succeeded}/${quarters.length} succeeded for ${companyId} (${fetched} fetched from MOPS, ${skipped} already in DB).`);
