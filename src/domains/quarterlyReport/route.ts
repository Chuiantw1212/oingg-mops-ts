 ...
 *       三支對外請求（MOPS）之間間隔隨機 5～10 秒（避免固定週期的請求被防火牆依規律性識別為機器人流量）；
 *       若某一表因為已在資料庫中而被跳過，該次不算對外請求，不會佔用等待時間，因此若三表都已存在，會在數百毫秒內完成。
 ...
 *       所有對外請求（最多 20*3=60 次）之間統一間隔隨機 5～10 秒（避免固定週期的請求被防火牆依規律性識別為
 *       機器人流量）；跳過的不佔用等待時間，因此若大部分季度資料庫已有資料，實際耗時會遠低於 60 次 x 5～10 秒。
 *       任一表/季度抓取失敗不會中斷其餘部分。
 *       進度會即時輸出到伺服器 console。
 *     tags:
 *       - Ingestion
