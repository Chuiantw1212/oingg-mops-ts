...
import { fetchCashFlow } from './service';
import { parseCashFlowReport } from './parser';
import type { CashFlowPayload } from './types';
...
export interface OneQuarterPayload {
  ...
  season: CashFlowPayload['season'];
  dataType: CashFlowPayload['dataType'];
...
// 向 MOPS 抓取單一季度現金流量表（累計數）、解析、並 upsert 進資料庫。單筆 API 與批次回補 API 共用此邏輯。
...
export const ingestOneQuarter = async (payload: OneQuarterPayload): Promise<IngestOneQuarterResult> => {
  ...
  try {
    if (!payload.force) {
      const existing = await prisma.quarterlyCashFlowStatement.findUnique({ where });
      if (existing) {
        return { success: true, skipped: true, ...meta, warnings: [], record: existing };
      }
    }

    const mopsResponse: MopsCashFlowResponse = await fetchCashFlow(payload);
    ...
    const { warnings, ...parsedFields } = parseCashFlowReport(mopsResponse.result.reportList);

    const record = await prisma.quarterlyCashFlowStatement.upsert({
      where,
      create: {
        symbol: payload.companyId,
        year: Number(payload.year),
